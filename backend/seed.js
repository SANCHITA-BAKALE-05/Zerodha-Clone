// One-off seed script: populates sample Holdings and Positions, then exits.
...
(async () => {
  try {
    ...
    const h = await HoldingsModel.countDocuments();
    const p = await PositionsModel.countDocuments();
    console.log(`Seeded ${h} holdings and ${p} positions.`);
  } catch (err) {
